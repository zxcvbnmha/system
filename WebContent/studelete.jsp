...
<%
  //接收学生姓名
    ...
    ResultSet rs=st.executeQuery("select * from 学生信息表    where 姓名='"+u+"'");
    
    //4.删除
    int flag = 1;
    if(rs.next()){
    sm.executeUpdate("delete  from 学生信息表    where 姓名='"+u+"'");
    }
    else{
    flag = 0;
   }
    ...
    <div style="margin-top:20px;">
    <input type="submit" value="返回继续删除" onclick="window.location.href='delete.jsp'"
    	   style="width:120px; height:35px;padding:5px 15px; border-radius:2px; border:1px solid;"/>
    </div>
